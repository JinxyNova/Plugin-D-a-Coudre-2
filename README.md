...
public final class ArenaConfig {
    ...
    // Matériau du fluide qui monte : WATER (eau) ou LAVA (lave)
    public Material materiauFluide;
    // Rayon (en blocs) autour de chaque joueur dans lequel le fluide est réellement posé.
    // Au lieu de remplir toute la zone (coûteux si la zone est grande), le fluide
    // "suit" les joueurs : seule la zone proche d'un joueur vivant est remplie.
    public int rayonFluide;
    ...
    public void charger() {
        ...
        String materiauBrut = cfg.getString("fluide.materiau", "WATER");
        Material materiau;
        try {
            materiau = Material.valueOf(materiauBrut == null ? "WATER" : materiauBrut.trim().toUpperCase());
        } catch (IllegalArgumentException e) {
            materiau = Material.WATER;
        }
        // Seuls l'eau et la lave sont supportées pour ce minigame
        materiauFluide = (materiau == Material.LAVA) ? Material.LAVA : Material.WATER;

        rayonFluide = cfg.getInt("fluide.rayon-joueur", 8);
        if (rayonFluide < 2) {
            rayonFluide = 2;
        }
    ...
    public void sauvegarder() {
        ...
        cfg.set("fluide.materiau", materiauFluide.name());
        cfg.set("fluide.rayon-joueur", rayonFluide);
